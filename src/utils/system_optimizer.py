...
class GrammarProcessor:
    ...
    def __init__(self, lang='en', structured_wordlist=None, wordlist=None, rules_path=None, knowledge_agent=None, *args, **kwargs):
        self.pos_map = defaultdict(lambda: 'NOUN')
        self._load_pos_data_async()
    ...
    def _load_pos_data(self):
        """Memory-efficient POS data loading"""
        ...
        try:
            ...
            logger.error(f"POS data loading failed: {str(e)}")
        
        return pos_mapping

    def _load_pos_data_async(self):
        """Background loading with memory limits"""
        import threading
        def loader():
            try:
                pos_path = Path(__file__).parent / "structured_wordlist_en.jsonl"
                with open(pos_path, 'r') as f:
                    for line in f:
        ...
        self.loader_thread = threading.Thread(target=loader)
