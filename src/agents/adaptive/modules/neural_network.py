...
from ..utils.adaptive_helpers import resolve_torch_device
...
class AdaptiveNetworkBase(nn.Module):
    ...
    def __init__(self, device: Optional[Union[str, torch.device]] = None) -> None:
        ...
        device_str = str(device) if device is not None else self.device_setting
    ...
    def _resolve_device(device_setting: str) -> torch.device:
        return resolve_torch_device(device_setting)
        ...
        try:
            with torch.no_grad():
                if init_name == "uniform_scaled":
                    ...
                    layer.weight.copy_(he_init(shape, nonlinearity="relu", device=device_str))
                elif init_name == "xavier_uniform":
                    layer.weight.copy_(xavier_uniform(shape, device=device_str))
                elif init_name == "lecun_normal":
                    layer.weight.copy_(lecun_normal(shape, device=device_str))
                else:
    ...
    def _normalize_layer_config(self, layers: Union[Sequence[int], Sequence[Dict[str, Any]]], *,
                                component: str, default_activation: str = "relu",) -> List[Dict[str, Any]]:
        ...
        *,
        ...
        if not torch.isfinite(tensor).all():
            raise InvalidValueError(
                ...
                component=component,
                details={"shape": tuple(tensor.shape)},
            )
...
class NeuralNetwork(AdaptiveNetworkBase):
    ...
    def __init__(
        ...
        if self.problem_type == "binary_classification":
            ...
            default_final_activation = "softmax"
        
        else:
            default_final_activation = "linear"
    ...
    def _configure_loss_function(self) -> nn.Module:
        """
        Configure the loss function while enforcing the output contract.
    
        Contracts
        ---------
        Regression:
            - Standard regression losses such as MSE, MAE, or Huber.
    
        Binary classification:
            output_dim == 1
                - BCEWithLogitsLoss
                - targets must be float tensors shaped [N, 1]
                - prediction uses sigmoid
    
            output_dim == 2
                - CrossEntropyLoss
                - targets must be class-index tensors shaped [N]
        ...
        loss_name = str(self.loss_name).lower().strip()
        ...
        if self.problem_type == "regression":
            ...
            if loss_name in {"huber", "smooth_l1"}:
                ...
                ensure_positive(
                    beta,
                    "neural_network.huber_beta",
                    component=NETWORK_COMPONENT,
                )
                return nn.SmoothL1Loss(beta=beta)
    
            raise UnsupportedLossError(
                f"Unsupported regression loss '{self.loss_name}'.",
                component=NETWORK_COMPONENT,
                details={
                ...
                remediation="Use one of: mse, mae, huber.",
            )
        ...
        if self.problem_type == "binary_classification":
            if self.output_dim == 1:
                if loss_name not in {
                    ...
                    raise UnsupportedLossError(
                        (
                            "Binary classification with output_dim=1 requires "
                            "BCEWithLogitsLoss."
                        ),
                        component=NETWORK_COMPONENT,
                        details={
                        ...
                        remediation=(
                            "Use 'bce_with_logits' for a single-logit binary "
                            "classifier."
                        ),
                    )
            ...
            if self.output_dim == 2:
                if loss_name not in {"cross_entropy", "ce"}:
                    raise UnsupportedLossError(
                        (
                            "Binary classification with output_dim=2 requires "
                            "CrossEntropyLoss."
                        ),
                        component=NETWORK_COMPONENT,
                        details={
                        ...
                        remediation=(
                            "Use 'cross_entropy' for a two-logit binary classifier."
                        ),
                    )
            ...
            raise NetworkConfigurationError(
                (
                    "binary_classification requires output_dim to be either "
                    "1 or 2."
                ),
                component=NETWORK_COMPONENT,
                details={
                ...
                remediation=(
                    "Configure the final layer with 1 neuron for BCEWithLogitsLoss "
                    "or 2 neurons for CrossEntropyLoss."
                ),
            )
        ...
        if self.problem_type == "multiclass_classification":
            if self.output_dim < 2:
                raise NetworkConfigurationError(
                    (
                        "multiclass_classification requires output_dim >= 2."
                    ),
                    component=NETWORK_COMPONENT,
                    details={
                        "problem_type": self.problem_type,
                    ...
                    remediation=(
                        "Configure the final layer with one output neuron "
                        "per class."
                    ),
                )
        ...
        raise InvalidConfigurationValueError(
            ...
            remediation=(
                "Use regression, binary_classification, or "
    ...
    def _configure_scheduler(self) -> Optional[torch.optim.lr_scheduler._LRScheduler]:
        ...
        if scheduler_name == "step":
            ...
            return torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=step_size, gamma=gamma)  # type: ignore[return-value]
        if scheduler_name == "cosine":
            t_max = int(scheduler_cfg.get("t_max", max(1, self.default_epochs)))
            eta_min = float(scheduler_cfg.get("eta_min", 0.0))
            return torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=t_max, eta_min=eta_min)  # type: ignore[return-value]
        if scheduler_name == "reduce_on_plateau":
            mode_str = str(scheduler_cfg.get("mode", "min")).lower().strip()
            mode = "min" if mode_str in ("min", "minimize") else "max"
    ...
    def train_network(
        ...
        for epoch in range(epochs):
            ...
            if val_inputs is not None and val_targets is not None:
                val_loss = self.evaluate(validation_data or [])
        ...
        run_train_losses: List[float] = []
        run_val_losses: List[float] = []
        epochs_ran = 0
        return {
            "epochs_ran": epochs_ran,
            "best_val_loss": None if best_val_loss == float("inf") else best_val_loss,
            "final_train_loss": run_train_losses[-1] if run_train_losses else None,
            "final_val_loss": run_val_losses[-1] if run_val_losses else None,
    ...
    def set_weights_biases(self, weights_biases: List[Dict[str, Any]]) -> None:
        ...
        linear_layers = [module for module in self.network.children() if isinstance(module, nn.Linear)]
    ...
    def _checkpoint_payload(self) -> Dict[str, Any]:
        """Prepare the checkpoint payload for saving the model state."""
        return {
            "class_name": self.__class__.__name__,
    ...
    def _restore_training_state(self, checkpoint: Mapping[str, Any]) -> None:
        ...
        self._is_fitted = bool(checkpoint.get("is_fitted", False))
        self.to(self.device)
        self.eval()
...
class BayesianDQN(NeuralNetwork):
    ...
    def _mc_predictions(self, state: Union[np.ndarray, torch.Tensor, Sequence[float]],
                        num_samples: Optional[int] = None) -> np.ndarray:
        """
        Generate Monte Carlo dropout predictions in the model's semantic
        output space.
    
        Classification:
            MC samples are converted from logits to probabilities before
            uncertainty statistics are calculated.
    
        Regression / Q-value outputs:
            Raw model outputs are retained.
    
        Dropout is enabled temporarily while the remainder of the model stays
        in evaluation mode.
        """
        sample_count = int(num_samples or self.num_uncertainty_samples)
        ensure_positive(sample_count, "num_samples",
                        component=BAYESIAN_COMPONENT)
    
        inputs = self._prepare_input_tensor(
            state,
        ...
        try:
            with torch.no_grad():
                for _ in range(sample_count):
                    # Do not call predict_tensor() here.
                    #
                    # predict_tensor() calls self.eval(), which would disable the
                    # dropout modules we intentionally enabled for MC inference.
                    outputs = self.network(inputs)
            ...
            raise wrap_exception(
                exc,
                ForwardPassError,
                "Monte Carlo dropout inference failed.",
                component=BAYESIAN_COMPONENT,
                details={
                    "input_shape": tuple(inputs.shape),
    ...
    def estimate_uncertainty(self, state: Union[np.ndarray, torch.Tensor, Sequence[float]],
                             num_samples: Optional[int] = None) -> np.ndarray:
        """
        Estimate predictive uncertainty using Monte Carlo dropout.
    
        Classification uncertainty is measured in probability space.
        Regression/Q-value uncertainty is measured in raw output space.
        """
        printer.status(
            "UNCERTAINTY",
        ...
        predictions = self._mc_predictions(
            state,
    ...
    def get_uncertainty_metrics(self, state: Union[np.ndarray, torch.Tensor, Sequence[float]],
                                num_samples: Optional[int] = None) -> Dict[str, Any]:
        """
        Return Monte Carlo prediction statistics in one consistent output space.
    
        For classification the returned mean, standard deviation, variance and
        confidence interval are all expressed in probability space.
    
        For regression and Q-value models they remain in raw model-output space.
        """
        predictions = self._mc_predictions(
            state,
            num_samples=num_samples,
        ...
        confidence_radius = (
            1.96 * std / np.sqrt(sample_count)
            if sample_count > 0
            else np.zeros_like(std)
        )
        ...
        return {
            ...
            "uncertainty_flag": bool(
                max_uncertainty > self.uncertainty_threshold
            ),
    ...
    def predict_with_uncertainty(self, inputs: Union[np.ndarray, torch.Tensor, Sequence[float]],
                                 num_samples: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        Return the Monte Carlo predictive mean and standard deviation.
    
        Both values are calculated from the same MC-dropout samples and therefore
        describe the same output space.
        """
        predictions = self._mc_predictions(
            inputs,
            num_samples=num_samples,
    ...
    def select_action(self, state: Union[np.ndarray, torch.Tensor, Sequence[float]], return_uncertainty: bool = False) -> Union[int, Tuple[int, Dict[str, Any]]]:
        ...
        mean_prediction = metrics["mean"]
        action = int(
            np.argmax(mean_prediction, axis=-1)
            .reshape(-1)[0]
        )
    ...
    def save_model(self, filepath: Union[str, Path]) -> Path:
        ...
        payload["class_name"] = self.__class__.__name__
...
class ActorCriticNetwork(AdaptiveNetworkBase):
    ...
    def _continuous_distribution(self, actor_output: torch.Tensor) -> torch.distributions.Normal:
        """
        Build the continuous-action distribution.
    
        The trainable action standard deviation is constrained to the configured
        range before constructing the distribution. This method is the single
        owner of continuous-action standard-deviation semantics.
        """
        action_std = ensure_not_none(
            self.action_std,
            "action_std",
            component=ACTOR_CRITIC_COMPONENT,
        )
        ...
        expanded_std = clamped_std.expand_as(actor_output)
    ...
    def _distribution_from_actor_output(
        self,
        actor_output: torch.Tensor,
    ) -> torch.distributions.Distribution:
        """
        Construct the policy distribution corresponding to actor output.
    
        Continuous policies use a Normal distribution whose mean is produced by
        the actor and whose standard deviation is owned by ActorCriticNetwork.
    
        Discrete policies interpret actor output as categorical logits.
        """
        if actor_output.ndim < 2:
            raise DimensionMismatchError(
                "Actor output must include batch and action dimensions.",
                component=ACTOR_CRITIC_COMPONENT,
                details={
                    "shape": tuple(actor_output.shape),
                    "action_dim": self.action_dim,
                },
            )
    
        ensure_dimension(
            int(actor_output.shape[-1]),
            self.action_dim,
            name="actor output dimension",
            component=ACTOR_CRITIC_COMPONENT,
        )
    
        if not torch.isfinite(actor_output).all():
            raise ForwardPassError(
                "Actor output contains NaN or infinite values.",
                component=ACTOR_CRITIC_COMPONENT,
                details={"shape": tuple(actor_output.shape)},
            )
    
        if self.continuous_action:
            return self._continuous_distribution(actor_output)
    ...
    def get_distribution(
        ...
        """
        Return the policy distribution for one state or a state batch.
    
        Distribution construction is intentionally owned by ActorCriticNetwork so
        callers do not reproduce discrete/continuous policy semantics.
        """
        actor_output = self.forward_actor(states)
    ...
    def sample_action(
        ...
        *,
        ...
        """
        Sample or deterministically select an action and return policy diagnostics.
        ...
        log_prob:
            Per-sample log probability with shape [batch].
    
        entropy:
            Per-sample policy entropy with shape [batch].
    
        value:
            Critic state-value estimate with shape [batch].
    
        Notes
        -----
        - Continuous exploration samples from the Normal policy.
        - Continuous deterministic execution uses the actor mean.
        - Discrete exploration samples from the categorical policy.
        - Discrete deterministic execution chooses argmax(logits).
        """
        actor_output, critic_output = self.forward(states)
    
        distribution = self._distribution_from_actor_output(
            actor_output,
        )
        ...
        if self.continuous_action:
            if not isinstance(
                ...
                raise NetworkConfigurationError(
                    "Continuous policy did not produce a Normal distribution.",
                    component=ACTOR_CRITIC_COMPONENT,
                )
            ...
            if not isinstance(
                ...
                raise NetworkConfigurationError(
                    "Discrete policy did not produce a Categorical distribution.",
                    component=ACTOR_CRITIC_COMPONENT,
                )
            ...
            action = (
                distribution.sample()
                if explore
                else torch.argmax(actor_output, dim=-1)
            )
    ...
    def get_action(self, states, #: Union[np.ndarray, torch.Tensor, Sequence[float]],
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Backward-compatible action-selection interface.
    
        New integrations that require entropy/value diagnostics should use
        sample_action().
        """
        action, log_prob, _, _ = self.sample_action(states=states, explore=True)
        return action, log_prob
    
    
    def get_value(self, states, #: Union[np.ndarray, torch.Tensor, Sequence[float]],
    ) -> torch.Tensor:
        """Return the critic value estimate for a state or state batch."""
    ...
    def evaluate_actions(self,
        ...
        -------
        log_probs:
            Per-sample action log probabilities with shape [batch].
    
        values:
            Per-sample critic values with shape [batch].
    
        entropy:
            Mean policy entropy across the batch.
    
        This method is the canonical training-time action-evaluation interface
        used by SkillWorker.
        """
        actor_output, critic_output = self.forward(states)
        ...
        batch_size = int(actor_output.shape[0])
    
        if self.continuous_action:
            actions_tensor = actions_tensor.float()
    
            # Single state, action vector:
            # [action_dim] -> [1, action_dim]
            if (
                actions_tensor.ndim == 1
                and batch_size == 1
            ):
                actions_tensor = actions_tensor.unsqueeze(0)
    
            # Batched scalar actions:
            # [batch] -> [batch, 1]
            elif (
                actions_tensor.ndim == 1
                and self.action_dim == 1
                and int(actions_tensor.shape[0]) == batch_size
            ):
                actions_tensor = actions_tensor.unsqueeze(-1)
    
            expected_shape = tuple(actor_output.shape)
    
            if tuple(actions_tensor.shape) != expected_shape:
                raise DimensionMismatchError(
                    "Continuous action tensor shape does not match actor output.",
                    component=ACTOR_CRITIC_COMPONENT,
                    details={
                        "expected": expected_shape,
                        "received": tuple(actions_tensor.shape),
                    },
                )
            ...
            entropy_values = distribution.entropy().sum(
                dim=-1
            )
            ...
            if int(actions_tensor.shape[0]) != batch_size:
                raise DimensionMismatchError(
                    "Discrete action batch size does not match state batch size.",
                    component=ACTOR_CRITIC_COMPONENT,
                    details={
                        "expected": batch_size,
                        "received": int(actions_tensor.shape[0]),
                    },
                )
    
            if torch.any(actions_tensor < 0) or torch.any(
                actions_tensor >= self.action_dim
            ):
                raise RangeValidationError(
                    "Discrete action contains an index outside the valid action range.",
                    component=ACTOR_CRITIC_COMPONENT,
                    details={
                        "action_dim": self.action_dim,
                    },
...
if __name__ == "__main__":
    ...
    try:
        ...
        (
            ...
            sampled_entropy,
            sampled_value,
        ) = actor_critic.sample_action(state_batch[0], explore=True)
        
        actions = np.random.randint(0, actor_critic.action_dim, size=(4,), dtype=np.int64)
        
        log_probs, values, entropy = (actor_critic.evaluate_actions(state_batch, actions))
        printer.status("TEST",(f"Sampled action: {action} | " f"log_prob: {log_prob} | "
                               f"entropy: {sampled_entropy} | " f"value: {sampled_value}"),
                               "success")
        printer.status("TEST",("Evaluate actions -> "
                               f"log_probs shape: {tuple(log_probs.shape)} | "
                               f"values shape: {tuple(values.shape)} | "
                               f"entropy: {float(entropy):.6f}"), "success",)
