...
class SGDRegressor:
    ...
    def __init__(self, config_override: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if config_override:
            self.sgd_config.update(config_override)
        self._load_config()
    ...
    def _compute_gradients(self, x_i: np.ndarray, y_i: float) -> Tuple[float, np.ndarray, float]:
        ...
        data_grad = error * x_i                     # no regularization
        intercept_grad = error if self.fit_intercept else 0.0
        return error, data_grad, intercept_grad
    ...
    def _sgd_epoch(self, X: np.ndarray, y: np.ndarray, sample_weight: np.ndarray) -> Dict[str, float]:
        ...
        for idx in indices:
            ...
            error, data_grad, intercept_grad = self._compute_gradients(x_i, y_i)
            # Scale data gradients by sample weight
            effective_data_grad = weight * data_grad
            effective_intercept_grad = weight * intercept_grad if self.fit_intercept else 0.0
            
            # Add regularization term (unscaled)
            total_grad_coef = effective_data_grad + (self.alpha * self.coef_ if self.coef_ is not None else 0.0)
            total_grad_intercept = effective_intercept_grad
            
            # Apply gradient clipping (if any) to the total gradients
            total_grad_coef, total_grad_intercept, grad_norm = self._apply_gradient_clipping(
                total_grad_coef, total_grad_intercept)
            gradient_norms.append(grad_norm)
            
            assert self.coef_ is not None
            coef_before = self.coef_.copy()
            intercept_before = self.intercept_
            self.coef_ -= self.eta * total_grad_coef
            if self.fit_intercept:
                self.intercept_ -= self.eta * total_grad_intercept
    ...
    def fit(
        ...
        return self.partial_fit(X, y, sample_weight=sample_weight, epochs=epochs)

    def partial_fit(self,
        X: Union[np.ndarray, Sequence[Sequence[float]], Sequence[float]],
        y: Union[np.ndarray, Sequence[float], float, int],
        sample_weight: Optional[Union[np.ndarray, Sequence[float]]] = None,
        *, epochs: Optional[int] = None) -> "SGDRegressor":
        """
        ...
        ----------
        epochs:
            Number of SGD epochs to execute for this update.

            When None, the configured max_iter is used, preserving the
            historical behaviour of partial_fit().

            Adaptive online consumers can explicitly request epochs=1 to
            perform one bounded update per newly observed batch.
        """
        ...
        try:
            ...
            epochs_to_run = (self.max_iter
                if epochs is None
                else int(epochs)
            )

            ensure_positive(epochs_to_run, "epochs", component="sgd_regressor")

            # Convergence describes the latest update run.
            self._converged = False

            for _ in range(epochs_to_run):
                metrics = self._sgd_epoch(X_arr, y_arr, weights)
                epoch_loss = float(metrics["epoch_loss"])
                converged = self._has_converged(epoch_loss, metrics["max_update_norm"])

                self._last_epoch_loss = epoch_loss
                self._last_gradient_norm = float(metrics["mean_gradient_norm"])
                self._last_update_norm = float(metrics["max_update_norm"])

                if self.track_history:
                    self.loss_history.append(epoch_loss)
                    self.learning_rate_history.append(float(self.eta))
            ...
            self.n_samples_seen += int(X_arr.shape[0])
            ...
            raise wrap_exception(
                ...
                component="sgd_regressor",
                details={"operation": "partial_fit", "epochs": epochs},
                remediation=("Inspect the input arrays and regressor "
    ...
    def mean_squared_error(self, X: Union[np.ndarray, Sequence[Sequence[float]], Sequence[float]],
                           y: Union[np.ndarray, Sequence[float], float, int]) -> float:
        X_arr, y_arr = self._validate_and_prepare_X_y(X, y)
        y_pred = self.predict(X_arr)
        return float(np.mean((y_arr - y_pred) ** 2))

    def mean_absolute_error(self, X: Union[np.ndarray, Sequence[Sequence[float]], Sequence[float]],
                            y: Union[np.ndarray, Sequence[float], float, int]) -> float:
        X_arr, y_arr = self._validate_and_prepare_X_y(X, y)
        y_pred = self.predict(X_arr)
    ...
    def load_model(cls, filepath: Union[str, Path]) -> "SGDRegressor":
        ...
        try:
            model = cls(config_override=payload.get("config"))
            state = payload["state"]
    ...
    def export_state(self) -> Dict[str, Any]:
        """
        Export learned and optimizer state for embedding in another checkpoint.
    
        The exported state contains all information required to continue
        incremental training from the same stochastic position.
        """
        return {
            "coef_": (
                None
                if self.coef_ is None
                else self.coef_.tolist()
            ),
            ...
            "learning_rate_history": [float(v) for v in self.learning_rate_history],
            "gradient_norm_history": [float(v) for v in self.gradient_norm_history],
            "update_norm_history": [float(v) for v in self.update_norm_history],
            "current_eta": float(self._current_eta),
            "best_loss": float(self._best_loss),
            "epochs_without_improvement": int(self._epochs_without_improvement),
            ...
            # Required for exact resumed stochastic training.
            "rng_state": self.rng.bit_generator.state,
        }

    def import_state(self, state: Mapping[str, Any]) -> None:
        """
        Restore a previously exported regressor state.
    
        Missing RNG state is accepted for compatibility with checkpoints
        created before RNG persistence was introduced.
        """
        ensure_instance(state, Mapping, "state", component="sgd_regressor")
        try:
            coef = state.get("coef_")
            self.coef_ = (
                None
                if coef is None
                else np.asarray(coef, dtype=np.float64))
            self.intercept_ = float(state.get("intercept_", 0.0))

            n_features = state.get("n_features_in_")
            ...
            self.n_samples_seen = int(state.get("n_samples_seen", 0))
            self.t_ = int(state.get("t_", 0))
            self.loss_history = [float(v) for v in state.get("loss_history", [])]
            self.learning_rate_history = [float(v) for v in state.get("learning_rate_history", [])]
            self.gradient_norm_history = [float(v) for v in state.get("gradient_norm_history", [])]
            self.update_norm_history = [float(v) for v in state.get("update_norm_history", [])]
            self._current_eta = float(state.get("current_eta", self.eta0))
            self._best_loss = float(state.get("best_loss", np.inf))
            self._epochs_without_improvement = int(state.get("epochs_without_improvement", 0))
            self._is_fitted = bool(state.get("is_fitted", self.coef_ is not None))
            self._last_epoch_loss = state.get("last_epoch_loss")
            self._last_gradient_norm = state.get("last_gradient_norm")
            self._last_update_norm = state.get("last_update_norm")
            ...
            raise StateImportError("Failed to import regressor state.",
                                   component="sgd_regressor", details={
                                       "state_keys": list(state.keys()),
                                       },
                                       remediation=("Ensure the provided state dictionary "
...
if __name__ == "__main__":
    ...
    # Generate the next values from the original RNG AFTER saving.
    # A correctly restored generator must produce exactly these values.
    expected_rng_values = regressor.rng.random(8)
    restored = SGDRegressor.load_model(checkpoint_path)
    restored_predictions = restored.predict(X_eval[:5])
    restored_rng_values = restored.rng.random(8)
    
    if not np.array_equal(expected_rng_values, restored_rng_values):
        raise AssertionError(
            "Restored SGDRegressor did not resume the "
            "saved NumPy RNG sequence."
        )
    
    printer.status("SAVE", f"Checkpoint saved to: {checkpoint_path}", "success")
    printer.status("LOAD", ("Restored prediction sample: " f"{restored_predictions}"), "success")
    printer.status("RNG", "Checkpoint RNG state restored successfully", "success")
