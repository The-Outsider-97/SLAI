...
class GradientBoostingHeuristic:
    ...
    def train(self, X, y):
        ...
        self.trained = True
        joblib.dump(self.model, 'gb_heuristic_model.pkl')

    def predict_success_prob(self, task, world_state, method_stats):
        """Predict plan success probability with confidence"""
        if not self.trained and os.path.exists('gb_heuristic_model.pkl'):
            self.model = joblib.load('gb_heuristic_model.pkl')
            self.trained = True
