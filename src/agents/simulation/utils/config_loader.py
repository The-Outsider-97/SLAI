...
"""
simulation subsystem configuration facade.

This module preserves the established simulation configuration API while
...
- the default path to ``simulation_config.yaml``;
- the simulation-facing compatibility API:
    - ``load_global_config``
    - ``reload_config``
    - ``clear_config_cache``
...
This preserves the existing simulation import surface without maintaining a
...
logger = get_logger("simulation Config Loader")
printer = PrettyPrinter()


# ---------------------------------------------------------------------------
# Canonical simulation configuration binding
# ---------------------------------------------------------------------------
...
def resolve_config_path(config_path: Optional[Union[str, Path]] = None) -> Path:
    """
    Resolve a simulation configuration path without reading the file.

    When ``config_path`` is omitted, the canonical
    ``simulation/configs/simulation_config.yaml`` path is returned.
...
def load_global_config(
    ...
    *,
    ...
    """
    Load the simulation subsystem configuration.

    This function preserves the established simulation API while delegating
    parsing, caching, modification detection, locking, and defensive copying
    to SLAI's shared configuration repository.

    Args:
        config_path:
            Optional custom simulation configuration path. When omitted,
            ``simulation_config.yaml`` is used.
    ...
    logger.debug("simulation configuration loaded | path=%s | force_reload=%s", resolved, force_reload)
...
def reload_config(config_path: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
    """
    Force-reload the simulation configuration.
    ...
    logger.info("simulation configuration reloaded | path=%s", resolved)
...
def clear_config_cache() -> None:
    """
    Clear configuration entries loaded through this simulation binding.
    ...
    logger.debug("simulation configuration cache cleared")


def get_config_cache_info() -> Dict[str, Any]:
    """
    Return cache diagnostics for the simulation configuration binding.
...
def get_config_section(
    ...
    *,
    ...
    """
    Return one top-level simulation configuration section.
    ...
    Args:
        ...
        config:
            Optional already-loaded simulation configuration.
