...
class RuleService(Protocol):
    ...
    def load_all_sectors(self) -> None:
        """Backward-compatible rule reload entry point."""
        ...

    def reload_rules(self) -> None:
        """Reload source-backed rules while preserving runtime rules."""
        ...
    ...
    def infer(self, knowledge_base: Mapping[Any, Any], *, sector: Optional[str] = None, smart: bool = True, trace: bool = False) -> InferenceResult:
        """Perform inference and return the canonical typed result."""
        ...
...
class KnowledgeOrchestrator(Protocol):
    """Contract for the top-level knowledge orchestrator."""
...
@dataclass
class KnowledgeOrchestratorState:
    """Runtime state for the knowledge orchestrator."""
...
@runtime_checkable
class KnowledgeSubsystem(Protocol):
    """Contract for the top-level knowledge subsystem."""

    def health(self) -> KnowledgeSubsystemHealth:
        ...

    def state(self) -> KnowledgeSubsystemState:
        ...


@dataclass 
class KnowledgeSubsystemState:
    """Runtime state for the knowledge subsystem."""

    orchestrator_state: KnowledgeOrchestratorState = field(default_factory=KnowledgeOrchestratorState)


@dataclass
class KnowledgeSubsystemHealth:
    """Runtime health envelope for the knowledge subsystem."""

    orchestrator_health: OrchestratorHealth


# ----------------------------------------------------------------------
# Unified export list – all symbols from errors + helpers + protocols
# ----------------------------------------------------------------------
__all__ = [
    "MemoryStore",
    "CacheStore",
    ...
    "KnowledgeSubsystem",
    "KnowledgeSubsystemState",
    "KnowledgeSubsystemHealth"
]
