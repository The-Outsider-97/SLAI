...
from ..utils.evaluation_errors import *
...
class EvaluationTransformer(BaseTransformer):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        This method matches the BaseTransformer override contract while preserving
        compatibility with the evaluation module's native ``x``/``attention_mask``
        calling convention. The ``src`` tensor is treated as the feature sequence,
        and legacy evaluation-only keyword arguments are accepted through kwargs.
        """
        attention_mask = kwargs.pop("attention_mask", None)
        return_sequence = bool(kwargs.pop("return_sequence", True))
        if kwargs:
            unexpected = ", ".join(sorted(kwargs))
            raise TypeError(f"Unexpected keyword argument(s): {unexpected}")

        # The evaluation stack is single-sequence: `src` is the feature tensor,
        # while `tgt` and the seq2seq masks are accepted only to satisfy the
        # base class signature and remain compatible with downstream callers.
        x = src
        if x is None:
            raise ValidationFailureError("inputs", None, "torch.Tensor")
    ...
    def reconstruct(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        encoded = self.encode_features(x, attention_mask=attention_mask)
        return self.reconstruction_head(encoded)

    def score(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    def classify(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    def estimate_risk(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, apply_sigmoid: bool = True) -> torch.Tensor:
    ...
    def compute_losses(self, batch: Mapping[str, torch.Tensor], reduction: str = "mean") -> Dict[str, torch.Tensor]:
        """
        ...
        outputs_raw = self.forward(inputs, attention_mask=attention_mask, return_dict=False)
        if not isinstance(outputs_raw, EvaluationTransformerOutput):
            raise TypeError("EvaluationTransformer.forward(..., return_dict=False) must return EvaluationTransformerOutput.")
        outputs = outputs_raw
    ...
    def summarize_batch(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> Dict[str, float]:
        """Return lightweight numeric diagnostics for a batch."""
        try:
            outputs_raw = self.forward(x, attention_mask=attention_mask, return_dict=False)
            if not isinstance(outputs_raw, EvaluationTransformerOutput):
                raise TypeError("EvaluationTransformer.forward(..., return_dict=False) must return EvaluationTransformerOutput.")
            outputs = outputs_raw
            summary = {
