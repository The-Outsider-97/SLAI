...
from typing import Any, Dict, Iterable, List, Literal, Mapping, MutableMapping, Optional, Sequence, Tuple, cast
...
class EvaluatorsCalculations:
    ...
    def __init__(self) -> None:
        ...
        self.average: Literal["micro", "macro", "samples", "weighted", "binary"] | None = cast(
            Literal["micro", "macro", "samples", "weighted", "binary"] | None,
            self.perform_config.get("average", "macro"),
        )
        ...
        for index, first_name in enumerate(names):
            for second_name in names[index + 1 :]:
                ...
                effects[key] = self.effect_size(
                    cast(Sequence[float], datasets[first_name]),
                    cast(Sequence[float], datasets[second_name]),
                )
        return effects
    ...
    def _calculate_power_analysis(self, datasets: Mapping[str, np.ndarray]) -> Dict[str, float]:
        ...
        for name, data in datasets.items():
            ...
            effect = self.effect_size(
                cast(Sequence[float], data),
                cast(Sequence[float], zero_baseline),
            )
