...
class EvidenceRecord:
    ...
    def __post_init__(self) -> None:
        parsed = _coerce_timestamp(self.timestamp)
...
class CertificationManager:
    ...
    def __init__(self) -> None:
        ...
        configured_domain = self.certification_config.get("domain")
        ...
        if not isinstance(configured_domain, str) or not configured_domain.strip():
...
class CertificationAuditor:
    ...
    def evaluate_asil(self, coverage: float, test_count: int) -> str:
        ...
        """
        coverage_value = _coerce_probability(coverage, "coverage", inclusive_zero=True, inclusive_one=True)
    ...
    def integrate_nist_rmf(self, metrics: Mapping[str, float]) -> List[str]:
        ...
        fairness_score = _coerce_probability(
            metrics.get("fairness_score", 1.0),
            "fairness_score",
            inclusive_zero=True,
            inclusive_one=True,
        )
        residual_risk = _coerce_probability(
            metrics.get("residual_risk", 0.0),
            "residual_risk",
            inclusive_zero=True,
            inclusive_one=True,
        )
