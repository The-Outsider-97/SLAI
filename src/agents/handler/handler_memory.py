...
class HandlerMemory:
    ...
    def _mirror_checkpoint(self, checkpoint: Mapping[str, Any]) -> None:
        if not self.mirror_to_shared_memory or self.shared_memory is None:
            return
        checkpoint_id = str(checkpoint.get("id"))
    ...
    def _should_mirror(self, mirror: Optional[bool]) -> bool:
        return self.mirror_to_shared_memory if mirror is None else bool(mirror)
