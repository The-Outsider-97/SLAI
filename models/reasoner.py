...
# --- Templates for Code Execution (from input_file_16.py) ---
...
def remove_comments_and_docstrings(code: str) -> str:
    ...
    try:
        ...
        # Basic comment removal (lines starting with #)
        # This won't handle block comments or inline comments perfectly.
        no_comments = "\n".join([line for line in code.splitlines() if not line.strip().startswith("#")])
        return no_comments # Docstring removal is harder without AST here.
...
class PythonExecutor:
    ...
    def _execute_code_unsafe(self, code_to_run: str) -> Tuple[Any, str]:
        ...
        # This is still not true sandboxing.
        restricted_globals = {
            "__builtins__": {
                ...
                "KeyError": KeyError, "NameError": NameError, "ZeroDivisionError": ZeroDivisionError,
                "Exception": Exception
            },
            # Allow specific safe modules if absolutely necessary for problem domain
             "copy": copy, # For deepcopy in some solutions
        }
...
class BasicZeroReasoner:
    ...
    """
    def init(self, executor_timeout: int = 5,
        banned_keywords: Optional[List[str]] = None,
        banned_assertion_keywords: Optional[List[str]] = None):
        self.executor = PythonExecutor(timeout_length=executor_timeout)
        self.generated_data = {
        "gen_code_i": [], # Successful items: {'snippet', 'input', 'target_output', 'imports', 'executed_output'}
        ...
        "gen_code_f": [], # Successful items: {'original_snippet', 'generated_inputs', 'generated_message', 'imports', 'executed_outputs_for_gen_inputs'}
        }
        self.banned_keywords = banned_keywords if banned_keywords is not None else DEFAULT_BANNED_KEYWORDS
        self.banned_assertion_keywords = banned_assertion_keywords if banned_assertion_keywords is not None else DEFAULT_BANNED_ASSERTION_KEYWORDS
