...
// Configuration
const PORT = Number(process.env.PORT || process.env.GAMES_PORT || 8000);
const PYTHON_PORT = Number(process.env.PYTHON_PORT || 5001);
...
async function startServer() {
  ...
  const startPythonProcess = () => {
    ...
    console.log(`Starting Python AI Server on http://127.0.0.1:${PYTHON_PORT} ...`);
    ...
    pythonProcess.on('close', (code) => {
      ...
      pythonProcess = null;
      if (!shuttingDown) {
        console.log('Restarting Python AI Server in 1s...');
        restartTimer = setTimeout(startPythonProcess, 1000);
      }
    });
  };

  // 1. Start and supervise Python AI server process.
  startPythonProcess();
  ...
  const server = app.listen(PORT, '0.0.0.0', () => {
    console.log(`Chronos web server running on http://localhost:${PORT}`);
    console.log(`Chronos AI proxy target: http://127.0.0.1:${PYTHON_PORT}`);
  });
