...
# iter_json_records() also performs structural validation as a second line of
# defence for previously unseen metadata filenames.
SUPERVISED_METADATA_JSON_NAMES = frozenset({
    "manifest.json",
...
def iter_json_records(path: Path) -> Iterator[Tuple[Mapping[str, Any], int]]:
    ...
    if suffix == ".json":
        ...
        if isinstance(payload, Mapping):
            wrapped_records: Any = None
            wrapper_name: Optional[str] = None
            for candidate in ("records", "examples", "data"):
                if candidate in payload:
                    wrapped_records = payload[candidate]
            ...
            if wrapper_name is not None:
                ...
                # A single supervised training example stored as a JSON object.
                records = [payload]
            else:
                # Metadata/provenance manifests are allowed to live beside the
                # JSONL task files.  They are not training examples and must not
                # be normalized as one merely because they are JSON objects.
                LOGGER.info(
                    "Skipping non-training JSON metadata file %s: top-level object has "
                    "no 'task' and no records/examples/data collection.",
                    path,
                )
                return
        else:
    ...
    for candidate in candidates:
        ...
        if candidate.is_file():
            discovered = (
                ...
                and candidate.name.lower() not in SUPERVISED_METADATA_JSON_NAMES
                else []
            )
        else:
            discovered = sorted(
                ...
                if item.is_file()
                and item.suffix.lower() in {".json", ".jsonl"}
                and item.name.lower() not in SUPERVISED_METADATA_JSON_NAMES
            )
...
def train_epoch(
    ...
    *,
    ...
    checkpoint_callback: Optional[Callable[[int, int, EpochStats, float], None]] = None,
    ...
    for index, example in enumerate(ordered, 1):
        ...
        if should_step:
            ...
            if (
                ...
                and global_optimizer_step % config.checkpoint_every_steps == 0
            ):
                checkpoint_callback(epoch, global_optimizer_step, stats, lr)
...
def save_checkpoint(
    ...
    *,
    ...
    if extra_metadata:
        metadata["training_progress"] = dict(extra_metadata)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=int(os.getenv("SLAI_LANTRA_CHECKPOINT_EVERY_STEPS", "1000")),
        help=(
            "Save a recoverable checkpoint every N optimizer steps in raw and supervised training. "
            "Default: 1000. Use 0 to disable interval checkpoints. Best/latest epoch checkpoints remain enabled."
        ),
    )
    ...
    try:
        ...
        if needs_raw_corpus:
            ...
            if raw_corpus.segments:
                ...
                PRINTER.status(
                    "LANTRA RAW",
                    "No raw-text corpus found; raw-text pretraining will be skipped.",
                    "info",
                )
        else:
            raw_files = []
            raw_corpus = empty_raw_corpus()
            PRINTER.status(
                "LANTRA RAW",
                "Raw corpus ingestion skipped because --raw-pretrain-epochs=0 and tokenizer retraining is disabled.",
                "info",
            )
