...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        # Upload Media Button
        self.upload_media_button = QtWidgets.QPushButton("Upload Media")
        self.upload_media_button.setFixedSize(125, 30)
        self.upload_media_button.setStyleSheet("background-color: #38b6ff; color: #0e0e0e; font-weight: bold; border-radius: 6px;")
        ...
        header_layout.addWidget(self.upload_media_button) # Add the Upload Media button
    ...
    def greetUser(self):
        ...
        # Post greeting to first tab's output box
        first_tab = self.tabs[0]  # First tab is index 0
        first_tab.setText(f"SLAI: {greeting_message}\n{'-'*40}\n")
    ...
    def upload_media(self):
        ...
        file_path, _ = file_dialog.getOpenFileName(
            ...
            "Select Media File",
            "",
            "Image Files (*.png *.jpg *.jpeg);;Audio Files (*.mp3 *.wav)"
        )
        ...
        if file_path:
            ...
            if file_extension in (".png", ".jpg", ".jpeg"):
                ...
                self.status_message.setText("Unsupported file type.")
                self.status_message.show()
                QTimer.singleShot(3000, self.status_message.hide)

    def display_image(self, image_path):
        """Displays the selected image in the current tab."""
        current_tab = self.tabs[self.tab_stack.currentIndex()]
        current_tab.clear()  # Clear previous content
        ...
        image = image.scaled(current_tab.viewport().size(), Qt.KeepAspectRatio)
