...
def generate_research_pdf_report(filepath, chapter_title, data):
    watermark_logo = data.get('watermark_logo_path', 'logo1.png')
    cover_logo = data.get('cover_logo_path', 'logo2.png')
    ...
    story.append(Paragraph("<b>SLAI</b>", styles['Title']))
    story.append(Spacer(1, 95 * mm))
    story.append(Paragraph(f"<b>Report Title:</b> Research Model Output", styles['Normal']))
    story.append(Paragraph(f"<b>Generated On:</b> {data['generated_on']}", styles['Normal']))
    story.append(Paragraph(f"<b>Generated By:</b> {data['generated_by']}", styles['Normal']))
    story.append(PageBreak())
    ...
    # === Chapter 1.1: Summary and Response ===
    chapter_ref = "chapter_1_1"
    chapter_title = f"Chapter 1.1: {chapter_title}"
    heading = Paragraph(f"<font size=14><b>{chapter_title}</b></font>", styles['ChapterTitle'])
    heading._bookmarkName = chapter_ref
    toc.addEntry(0, chapter_title, chapter_ref)
    story.append(NextPageTemplate('with_footer'))
    story.append(PageBreak())
    story.append(KeepTogether([heading]))
    ...
    story.append(Paragraph(data["response"], styles['BodyText']))
    story.append(PageBreak())

    # === Chapter 1.2: Metadata and Sources ===
    subchapter_ref = "chapter_1_2"
    subchapter_title = "Chapter 1.2: Metadata and Agent Trace"
    sub_heading = Paragraph(f"<font size=14><b>{subchapter_title}</b></font>", styles['ChapterTitle'])
    sub_heading._bookmarkName = subchapter_ref
    toc.addEntry(0, subchapter_title, subchapter_ref)
    story.append(KeepTogether([sub_heading]))
    story.append(Spacer(1, 4 * mm))

    story.append(Paragraph("<b>Sources (APA Style):</b>", styles['Heading2']))
    for source in data["sources"]:
        story.append(Paragraph(source, styles['BodyText']))
    story.append(Spacer(1, 6 * mm))

    story.append(Paragraph("<b>Time Taken:</b>", styles['Heading2']))
    story.append(Paragraph(data["time_taken"], styles['BodyText']))
    story.append(Spacer(1, 6 * mm))

    story.append(Paragraph("<b>Follow-Up Question:</b>", styles['Heading2']))
    story.append(Paragraph(data["follow_up_question"], styles['BodyText']))
    story.append(Spacer(1, 6 * mm))

    story.append(Paragraph("<b>Agent Trace:</b>", styles['Heading2']))
    for step in data["agent_trace"]["steps"]:
        step_info = f"{step['stage'].capitalize()}: {str(step['value'])}"
        story.append(Paragraph(step_info, styles['BodyText']))
        story.append(Spacer(1, 2 * mm))
